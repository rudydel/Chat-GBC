...
### Intro screen and banking

Bank 0 is nearly full (inference code, the unrolled dot product and the
square table), so the intro screen (`gb/src/intro.c`: logo, title, menu,
credits and the history viewer) is compiled with `#pragma bank 255` and the
ROM is linked with `-autobank`: GBDK's `bankpack` puts it into whichever
weight bank has room, so the cartridge size does not change with the model.
`intro_run()` is a `BANKED` function; everything it draws goes through the
bank-0 text routines in `ui.c`.

The graphics (`gb/src/intro_gfx.h`: 122 unique 2bpp tiles plus the tile
maps of the 14x8 logo and the 16x2 title) are generated by
`python3 tools/introgen.py [preview.png]`. The cat is drawn from a few
geometric shapes in the script, the title and the menu frame are pixel art
in it; the optional argument writes a PNG of the whole screen. The header is
included by `intro.c` only, on purpose: bankpack assigns banks per object
file, and the tiles must sit in the bank that is mapped while the code that
copies them to VRAM runs. The tiles are loaded after the 114 font tiles, so
the menu and the credits use the normal font.

The chat history is a ring of the last 96 finished log lines (`ui.c`,
`HIST_LINES`, 1.9 KiB of WRAM), recorded while a question is answered; it
lives in work RAM, so it lasts until the console is switched off.
...
* `tools/emu_test.py` uses the normal ROM: it picks "new chat" on the intro
  menu, navigates the on-screen keyboard, types the prompt, presses START,
  reads the reply back from the tile map and compares it with the simulator.
...
* The header flags the ROM as CGB compatible (`-Wm-yc`), so a Game Boy Color
  ...
  palettes white, so `ui_init()` loads BG palette 0 with the four greens of
  the original Game Boy screen when `_cpu == CGB_TYPE`. Without that the ROM
